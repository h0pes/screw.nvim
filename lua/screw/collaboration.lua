...
function M.setup()
  ...
  -- Log the result (only if not already logged for explicit config)
...
function M.setup_realtime_sync()
  ...
  if backend.__class == "HttpBackend" then
    utils.info("HTTP collaboration sync enabled")
...
function M.check_connection_health()
  ...
  if not status.is_listening then
    ...
    vim.defer_fn(function()
      local success, error_msg = state.realtime_sync:start()
      if success then
        utils.info("Real-time sync reconnected successfully")
      else
