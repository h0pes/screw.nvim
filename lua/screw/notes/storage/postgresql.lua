...
function PostgreSQLBackend:recover_from_offline_mode()
  ...
  utils.info("Recovering from offline mode - processing " .. #self.offline.queued_operations .. " queued operations")
...
function PostgreSQLBackend:enter_offline_mode(reason)
  ...
  self.offline.active = true
  utils.warn("Entering offline mode: " .. reason)
  utils.info("Note operations will be queued and synchronized when connection is restored")
...
function PostgreSQLBackend:attempt_reconnection()
  ...
  local success, error_msg = self:connect()
  if success then
    utils.info("Database connection restored")
    return true
  else
